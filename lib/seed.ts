...
function build(id: string, name: string, businessUnit: string, type: ValueTree["type"], rootId: string, seeds: Seed[]): ValueTree {
  const nodes: Record<string, ValueNode> = {};
  for (const s of seeds) nodes[s.id] = { ...s, position: { x: 0, y: 0 } };
  return { id, name, businessUnit, type, rootId, nodes };
}
...
// ---------------------------------------------------------------------------
export const evaTree: ValueTree = build("eva-demo", "EVA Value Tree (2010)", "Corporate Finance", "financial", "eva", [
...
// ---------------------------------------------------------------------------
export const strategicHealthTree: ValueTree = build("health-demo", "Strategic Health Scorecard", "Strategy Office", "qualitative", "health", [
